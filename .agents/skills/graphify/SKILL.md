...
## llm-wiki repository convention

Before any pipeline step that writes `graphify-out/`, **`cd` to the library `work/` directory** (from the git root: `cd work`). Graphify emits `graphify-out/` relative to the current working directory, so this places artifacts in **`work/graphify-out/`**.

Use **input paths relative to `work/`**: `../raw` for immutable sources, `../wiki` for compiled markdown notes. If a single root is required per run, start with `../raw`; use `../wiki` with `--update` on a follow-up pass to merge into the existing graph.

Never write Graphify outputs into the canonical top-level **`wiki/`** tree. If using `--wiki`, keep that export under `work/graphify-out/` only.
